                        ...
                        <div className="space-y-4">
                            ...
                            {result && (
                                <div className="glass-card rounded-[32px] p-6 border border-[#22D97A]/20 space-y-5 animate-in fade-in duration-300">
                                    ...
                                    {advancedReport ? (
                                        ...
                                        <div className="bg-[#22D97A]/5 border border-[#22D97A]/20 rounded-[22px] p-4 space-y-3">
                                            ...
                                            {!advancedReport.insights.length && !advancedReport.recommendations.length && (
                                                <p className="text-xs text-[#A6B0B5]">Your basket has no additional rule-based recommendations.</p>
                                            )}
                                        </div>
                                        </>
                                    ) : (
                                    ...
                                    <div className="bg-[#22D97A]/5 border border-[#22D97A]/15 rounded-[22px] p-4 space-y-3">
                                        <p className="text-sm font-black text-[#22D97A] font-heading">Choose Intelligence APIs</p>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            {intelligenceOptions.map(({ id, name, price, handler, loaded }) => (
                                                <button key={id as string} onClick={handler as () => void} disabled={activeIntelligenceCall !== null} className="rounded-xl border border-[#22D97A]/15 bg-black/10 px-3 py-3 text-left hover:border-[#22D97A]/40 disabled:opacity-50">
                                                    <span className="block text-xs font-black text-white">{activeIntelligenceCall === id ? "Calling..." : loaded ? `${name} · Loaded` : name}</span>
                                                    <span className="block mt-1 text-[10px] text-[#A6B0B5]/60">{price} · separate x402 call</span>
                                                </button>
                                            ))}
                                        </div>
                            ...
                            <details className="group rounded-[24px] border border-[#22D97A]/15 bg-[#22D97A]/5 p-5">
                                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 [&::-webkit-details-marker]:hidden">
                                    <div>
                                        <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#22D97A]">For AI Agents</p>
                                        <p className="mt-1 text-sm font-bold text-white">Machine-readable Replate Intelligence API via x402</p>
                                    </div>
                                    <ChevronDown size={18} className="shrink-0 text-[#22D97A] transition-transform group-open:rotate-180" />
                                </summary>
                                <div className="mt-4 space-y-4 border-t border-[#22D97A]/10 pt-4">
                                    <p className="text-xs leading-5 text-[#A6B0B5]">
                                        ...
                                        {[
                                            ...
                                            <div key={endpoint} className="flex items-center justify-between gap-3 rounded-xl border border-white/5 bg-black/10 px-3 py-2.5">
                                                <div className="min-w-0">
                                                    ...
                                                    <code className="block truncate text-[10px] text-[#A6B0B5]/60">{endpoint}</code>
                                                </div>
                                                <div className="shrink-0 text-right">
                                                    <p className="text-xs font-black text-[#22D97A]">{price}</p>
                                                    <p className={`text-[10px] font-bold ${status === "Live" ? "text-[#22D97A]" : "text-[#A6B0B5]/40"}`}>{status}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    <div className="flex flex-wrap gap-3 text-xs font-bold">
                                        <a href={getApiUrl("/openapi.json")} target="_blank" rel="noreferrer" className="text-[#22D97A] hover:underline">OpenAPI spec →</a>
                                        <a href={getApiUrl("/.well-known/agent-card.json")} target="_blank" rel="noreferrer" className="text-[#22D97A] hover:underline">Agent card →</a>
                                    </div>
                                </div>
                            </details>
