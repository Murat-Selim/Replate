...
export async function submitReceiptToContract(data) {
    ...
    try {
        ...
        const tx = await c.submitReceipt(data.user, data.totalItems, data.healthyItems, data.unhealthyItems, data.fruitVegGrams, data.householdSize, data.daysCovered);
        console.log(`📤 Transaction sent: ${tx.hash}`);
        // Wait for confirmation
        const receipt = await tx.wait();
        console.log(`✅ Transaction confirmed in block ${receipt.blockNumber}`);
        // Parse events to get the result
        const receiptEvent = receipt.logs.find((log) => log.topics[0] === ethers.id("ReceiptSubmitted(address,uint8,uint8,uint256,uint16,uint16)"));
        const badgeMinted = !!receipt.logs.find((log) => log.topics[0] === ethers.id("BadgeMinted(address,uint256)"));
        if (receiptEvent) {
...
export async function finalizeUserWeek(userAddress) {
    ...
    try {
        ...
        const event = receipt.logs.find((log) => log.topics[0] === ethers.id("WeekFinalized(address,uint256,uint256,uint256)"));
        if (event) {
            const decoded = ethers.AbiCoder.defaultAbiCoder().decode(["uint256", "uint256", "uint256"], event.data);
...
function mockContractResponse(data) {
...
function getMockLeaderboard(limit) {
    const mock = [
